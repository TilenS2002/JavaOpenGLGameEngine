...
public class MouseInput {
    ...
    public void init() {
        GLFW.glfwSetCursorPosCallback(App.getWindow().getWindowHandler(), (window, xpos, ypos) -> {
            currentPos.x = xpos;
            currentPos.y = ypos;
        });

        GLFW.glfwSetCursorEnterCallback(App.getWindow().getWindowHandler(), (window, entered) -> {
            inWindow = entered;
        });

        GLFW.glfwSetMouseButtonCallback(App.getWindow().getWindowHandler(), (window, button, action, mods) -> {
